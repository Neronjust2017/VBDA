...
wandb.init(project="vbda_ddp", entity="yuhuawei", name="mc_2gpu_tran30")
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, model: ImageClassifier,
    ...
    for i in range(args.iters_per_epoch):
        ...
        # compute output
        ys_s = torch.zeros(x_s.shape[0], model.module.num_classes, num_ens).to(device)
        ys_t = torch.zeros(x_t.shape[0], model.module.num_classes, num_ens).to(device)
        fs_s = torch.zeros(x_s.shape[0], model.module.features_dim, num_ens).to(device)
        fs_t = torch.zeros(x_t.shape[0], model.module.features_dim, num_ens).to(device)
